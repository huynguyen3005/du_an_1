...
if (isset($_POST['login'])) {
   ...
   if (strlen($name) == 0) {
      $message['username'] = 'Please enter your email';
   }
      ...
      if ($res) {
         ...
         if (password_verify($password, $dbP)) {
            ...
            if (isset($_POST['remember'])) {
               add_cookie('user_name', $name, (3600 * 24 * 30));
               add_cookie('password', $password, (3600 * 24 * 30));
            } else {
            ...
            $message['password'] = 'The password incorrect';
         }
      } else {
         $message['username'] = 'The email incorect';
      }
      // die();
   }
}
                  ...
                  <div class="tp-login-option">
                     ...
                     <form action="" method="post">
                        <div class="tp-login-input-wrapper">
                           <div class="tp-login-input-box">
                              <div class="tp-login-input">
                                 <input id="email" type="text" placeholder="shofy@mail.com" name="username" value="<?= $_SESSION['register-email'] ?? '' ?>">
                              </div>
                              <div class="tp-login-input-title">
                                 <label for="email">Your Email</label>
                              </div>
                              <div class="<?= isset($message['username']) ? "alert" : "" ?> alert-danger">
                                 <?= $message['username'] ?? '' ?>
                              </div>
                           </div>
                        ...
                        <div class="tp-login-bottom">
                           <button class="tp-login-btn w-100" type="submit" name="login">Login</button>
                        </div>
                     </form>
                  </div>
               </div>
